  ...
  return (
    <article
      className={`rounded-2xl border p-6 transition-transform duration-200 hover:-translate-y-1 ${
        isDark
          ? 'bg-[#0F172A] border-white/10 shadow-[0_18px_40px_rgba(15,23,42,0.26)]'
          : 'bg-white border-[#E2E8F0] shadow-[0_4px_24px_rgba(0,0,0,0.06)]'
      }`}
    >
      <div
      ...
      <h3 className={`text-xl font-semibold tracking-[-0.02em] ${isDark ? 'text-white' : 'text-[#0F172A]'} ${localeTextClass}`}>
        {card.title}
      </h3>
      <p className={`mt-3 text-base leading-7 ${isDark ? 'text-white/72' : 'text-[#64748B]'} ${localeTextClass}`}>
        {card.body}
      </p>
    </article>
  )
  ...
  return (
    <section className="mb-10 space-y-16 lg:space-y-20">
      ...
      <section id="preview" className="landing-anchor-offset space-y-6">
        ...
        <div className="overflow-hidden rounded-2xl border border-white/8 bg-[#0F172A] shadow-[0_18px_60px_rgba(15,23,42,0.28)]">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[0.92fr,1.08fr]">
            <div className="border-b border-white/8 bg-[#1E293B] p-5 lg:border-b-0 lg:border-r">
              ...
              <div className="mt-4 space-y-3">
                {activePreview?.sourceLines.map((line) => (
                  <p key={line} className={`text-[15px] leading-7 text-[#94A3B8] ${localeTextClass}`}>
                    {line}
                  </p>
                ))}
              </div>
            ...
            <div className="bg-[#0F172A] p-5">
              ...
              <div className="mt-4 space-y-4">
                {activePreview?.outputSections.map((section) => (
                  <div key={section.title} className="rounded-2xl border border-white/8 bg-white/[0.02] p-4">
                    <p className={`text-sm font-semibold text-white ${localeTextClass}`}>{section.title}</p>
                    <ul className="mt-3 space-y-2">
                      {section.items.map((item, itemIndex) => (
                        <li
                          key={item}
                          className={`mallog-output-item flex items-start gap-2 text-[15px] leading-7 text-white/74 ${localeTextClass}`}
                ...
                <div className="rounded-2xl border border-[#3B82F6]/20 bg-[#111C34] px-4 py-3 text-sm text-[#93C5FD]">
                  {locale === 'kr'
