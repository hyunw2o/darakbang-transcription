...
export default function Home({ darkMode, setDarkMode, uiTheme, setUiTheme, uiThemeMode, setUiThemeMode }) {
  ...
  const validateAndSetFile = async (selectedFile) => {
    ...
    try {
      ...
      setFileDurationSeconds(0)
      setError(null)
      setNotice('Could not read duration in browser. Upload continues and the server will validate duration.')
      setResult(null)
      setRecordDrafts({})
    }
  }
  ...
  const exportAsTxt = () => {
    ...
    if (!text) return
    const filename = `${sanitizeFileName(`transcript_${new Date().toISOString().slice(0, 10)}`)}.txt`
  ...
  const exportAsDocx = () => {
    ...
    const text = (result.corrected_text || result.raw_text || '').trim()
    if (!text) return
    const filename = `${sanitizeFileName(`transcript_${new Date().toISOString().slice(0, 10)}`)}.docx`
    const blob = buildDocxBlob('Transcript', text)
    triggerBlobDownload(blob, filename)
  }
