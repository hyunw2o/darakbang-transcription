...
4. 환경변수 설정
   ...
   - `OPENAI_TRANSCRIPTION_FALLBACK_MODELS` (기본 `whisper-1`, 기본 전사 모델 실패 시 폴백 모델 목록)
   - `OPENAI_TRANSCRIPTION_TEMPERATURE` (기본 0, 전사 결과의 무작위성 최소화)
   - `WHISPER_PREPROCESS_SAMPLE_RATE` (기본 24000, 전사용 오디오 샘플레이트)
   - `WHISPER_PREPROCESS_BITRATE` (기본 `64k`, 전사용 MP3 비트레이트)
   - `WHISPER_PREPROCESS_LOWPASS_FREQ` (기본 11000, 자음 손실을 줄이기 위한 lowpass 주파수)
   ...
   - `WHISPER_SOURCE_OPENING_GUARD_SECONDS` (기본 30초, 원본 파일 시작부를 별도 재인식해 전처리 전 첫 발화 누락을 보완)
